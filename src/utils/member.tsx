...
export function compareMemberArrays(array1: IContributor[], array2: IContributor[]) {
	...
	const addMemberList = array2.filter((item) => !set1.has(item.wallet)).map(item => item.wallet);
	const removeMemberList = array1.filter((item) => !set2.has(item.wallet)).map(item => item.wallet);
	...
	const addAdminList = adminArr2.filter(item => !adminSet1.has(item.wallet)).map(item => item.wallet);
	const removeAdminList = adminArr1.filter(item => !adminSet2.has(item.wallet)).map(item => item.wallet);
